...
def fit_polynomial_sgd(x_t_pairs: torch.Tensor, m: int, lr=0.0001, mb_size=2) -> torch.Tensor:
    """
...
def fit_polynomial_sgd_and_test(x_t_test_set_pairs: torch.Tensor, x_t_pairs: torch.Tensor, m: int,
                                lr=0.0001, mb_size=2) -> torch.Tensor:
    """
    Implement stochastic minibatch gradient descent for fitting polynomial functions for given dataset,
    to given degree (plus bias).
    ...
    for epoch in range(num_epochs):
        ...
        # "report the loss periodically using printed messages"
        if epoch % print_epochs == 0:
            print(f'Epoch [{epoch + 1} / {num_epochs}], Loss: {round_sig_figs(to_round=loss.item(), sf=4)}')
...
def rmse(w_or_y_true: torch.Tensor, w_or_y_pred: torch.Tensor) -> float:
    """
    Calculate root-mean-square error (RMSE) for given weights `w` or predicted dependent variable `y`, to 4
    significant figures.
    :param w_or_y_true: Ground-truth weights or predicted values. Weights shapes (3,1), (4,1) or (5,1). Predicted y
    values shapes (10, 1).
    :param w_or_y_pred: Computed weights or predicted values. Weights shapes (3,1), (4,1) or (5,1). Predicted y values
    shapes (10, 1).
    :return: Root-mean-square error (RMSE), to 4 significant figures.
    """
    size_diff = w_or_y_pred.shape[0] - w_or_y_true.shape[0]
    if size_diff > 0:
        zeros = torch.zeros(size_diff).unsqueeze(dim=1)
        w_or_y_true = torch.cat((w_or_y_true, zeros), dim=0)
    rmse = torch.sqrt(torch.mean((w_or_y_true - w_or_y_pred) ** 2))
    rmse = round_sig_figs(to_round=float(rmse), sf=4)
    return rmse


def round_sig_figs(to_round: float, sf: int) -> float:
    """
    Round given number to given significant figures.
    :param to_round: Float to be rounded.
    :param sf: Number of significant figures to round to.
    :return: Given number rounded to specified number of significant figures.
    """
    if to_round < 0:
        rounded_x = round(to_round, -int(floor(log10(-to_round))) + (sf - 1))
    elif to_round > 0:
        rounded_x = round(to_round, -int(floor(log10(to_round))) + (sf - 1))
    else:
        rounded_x = 0
